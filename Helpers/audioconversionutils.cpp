...
AudioConversionUtils::AudioConversionUtils()
{
    ...
    for (int i = 0; i < 9; i++)
    {
        m_spikeConvFunction[i] = (-4.0f + i) * 0.25f;
        m_spikeConvFunction[17 - i] = m_spikeConvFunction[i];
    }
}
...
void AudioConversionUtils::debugAudioFormat(const QAudioFormat &audioFormat)
{
    ...
    {
    case QAudioFormat::SampleFormat::Int16: str += "Int16"; break;
    case QAudioFormat::SampleFormat::Int32: str += "Int32"; break;
    case QAudioFormat::SampleFormat::UInt8: str += "UInt8"; break;
    case QAudioFormat::SampleFormat::Float: str += "Float"; break;
...
bool AudioConversionUtils::convertSamplesToFloat(const QAudioFormat &format, const char *data, size_t dataSize, QVector<float> &out)
{
    ...
    {
        normalizeType<int16_t>(format, data, dataSize, out);
        return true;
    }
    case QAudioFormat::SampleFormat::Int32:
    {
        normalizeType<int32_t>(format, data, dataSize, out);
        return true;
    }
    case QAudioFormat::SampleFormat::UInt8:
    {
        normalizeType<uint8_t>(format, data, dataSize, out);
        return true;
    }
...
void AudioConversionUtils::normalizeType(const QAudioFormat &format, const char *data, size_t dataSize, QVector<float> &out)
{
    ...
    normalizeAudio<Type>(reinterpret_cast<const Type*>(data), out.data(), dataSize / sizeof(Type), true);
}
